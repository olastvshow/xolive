...
export default function TennisGame(){
 ...
 return <section className="sports-fullscreen" aria-label="Table Tennis match"><div className="relative z-10 mx-auto grid max-w-lg grid-cols-[auto_1fr_auto] items-start gap-4 px-6 py-4 text-sm text-ink"><strong>You · {s.scores[p.me.id]}</strong><span className="text-center">{p.live&&!p.partnerOnline?'Paused · opponent disconnected':s.seq===0?'Hold to toss · swipe up for topspin, down for backspin':`${s.format} · ${s.target} points`}</span><strong>{s.scores[p.partner.id]} · {p.partner.display_name}</strong></div><div ref={stage} className="sports-touch-stage touch-none"><Stage3D camera={[0,2.6,6.4]} fov={50} background="#202c35"><Scene value={value} input={input.current} juice={juice.current}/></Stage3D></div><div className="sports-exit"><Button variant="ghost" onClick={()=>void p.leaveGame()}>Leave game</Button></div></section>;
}
