...
import { shotFromFlick } from './motion';
...
export default function SportsGame(){
 ...
 useEffect(()=>{const el=stage.current;if(!el)return;
    const shoot=(keyboard=false)=>{const v=value.current,state=v.state as SportsState;if(v.busy||state.done||state.turn!==v.me.id||(v.live&&!v.partnerOnline)||(state.last&&elapsed.current<(state.last.duration??0)+.9))return;const i=input.current;
     const g=shotFromFlick(i.vx,i.vy);
     if(!g.valid&&!keyboard)return; // a tap or a stray touch is never a throw
     v.play({type:'throw',seq:state.seq,aim:g.aim,power:keyboard&&!g.valid?clamp(-i.vy,0,1):g.power,origin:clamp((i.x-.5)*2.4,-1.2,1.2)});};
   const clean=input.current.bind(el,()=>shoot(),()=>juice.current.unlock());const key=(e:KeyboardEvent)=>{if((e.target as HTMLElement).tagName==='INPUT')return;if(e.key==='ArrowLeft')input.current.vx-=.1;if(e.key==='ArrowRight')input.current.vx+=.1;if(e.key==='ArrowUp')input.current.vy-=.15;if(e.key==='ArrowDown')input.current.vy+=.15;if(e.code==='Space'){e.preventDefault();shoot(true);}};window.addEventListener('keydown',key);return()=>{clean();window.removeEventListener('keydown',key);};
 },[s.phase]);
